...
public class HUDMobileChatEventStream : MonoBehaviour
{
	...
	private void Start()
	{
		...
		{
			GameObject gameObject = GameObjectHelper.Instantiate(template.gameObject, template.transform.parent, new Vector3(0f, (float)num * ySpace, 0f));
	...
	{
		string text = GUIUtils.ColorToNGuiModifier(ColorScheme.GetNameColorByAccessLevel(accessLevel));
		items.Add(new Item
		{
