...
public static class GameAudio
{
	...
	static GameAudio()
	{
		...
		try
		{
			...
			if (go == null)
			{
				Debug.LogWarning("GameAudio: GameObject not found in scene — audio will be silent until clips are configured.");
				return;
			}
			component = go.GetComponent<AudioClipConfigurator>();
			if (component == null || component.Assets == null || component.Assets.Count == 0)
			{
				Debug.LogWarning("GameAudio: AudioClipConfigurator has no clips assigned — audio will be silent.");
				return;
			}
		}
		catch
		{
			Debug.LogWarning("GameAudio: Could not initialize — audio will be silent.");
			return;
		}
