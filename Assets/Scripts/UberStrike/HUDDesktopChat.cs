...
public class HUDDesktopChat : MonoBehaviour
{
	...
	private void Start()
	{
		...
		{
			GameObject gameObject = GameObjectHelper.Instantiate(template.gameObject, template.transform.parent, new Vector3(0f, (float)num * ySpace, 0f), template.transform.localScale);
	...
	private void OnInputChanged(object input = null)
	{
		...
		float num = NGUIMath.CalculateRelativeWidgetBounds(inputLabel.transform).size.y * inputLabel.transform.localScale.y;
