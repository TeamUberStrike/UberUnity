...
public class ReportPlayerPanelGUI : PanelGuiBase
{
	...
	public static void ReportInboxPlayer(PrivateMessageView msg, string messageSender)
	{
		...
		if (AutoMonoBehaviour<CommConnectionManager>.Instance.Client.IsConnected)
		{
			...
			{
				AutoMonoBehaviour<CommConnectionManager>.Instance.Client.Lobby.Operations.SendPlayersReported(new List<int> { reportedCmid }, 0, reason, Singleton<ChatManager>.Instance.GetAllChatMessagesForPlayerReport());
	...
	private void DrawReportPanel()
	{
		...
		GUI.enabled = true;
		int num = DoDropDownList(new Rect(125f, 20f, 183f, 22f), new Rect(135f, 50f, 194f, 84f), _reportTypeTexts, ref _abuse, false);
		if (num != -1)
		{
