	...
	public void Update()
	{
		if (ApplicationDataManager.ApplicationOptions.VideoMotionBlur)
		{
			...
			if (_effectsParameters.TryGetValue(ImageEffectType.MotionBlur, out value) && value != null && value.EffectEnable)
			{
				...
				{
//					((MonoBehaviour)_effects[ImageEffectType.MotionBlur]).blurAmount = 0.5f;
				}
...
//					((MonoBehaviour)_effects[ImageEffectType.MotionBlur]).blurAmount = _effectsParameters[ImageEffectType.MotionBlur].ActiveTime / _effectsParameters[ImageEffectType.MotionBlur].TotalTime * baseIntencity;
				}
			}
		}
		...
		if (_effects.ContainsKey(ImageEffectType.MotionBlur))
		{
			_effects[ImageEffectType.MotionBlur].enabled = ApplicationDataManager.ApplicationOptions.VideoBloomAndFlares;
		}
	}
	...
	public void EnableEffect(ImageEffectType imageEffectType, float duration, float intensity)
	{
		if (_effects.ContainsKey(imageEffectType) && _effectsParameters.ContainsKey(imageEffectType))
		{
			...
			if (imageEffectType == ImageEffectType.MotionBlur)
			{
//				_effectsParameters[imageEffectType].SetBaseIntensity(((MonoBehaviour)_effects[ImageEffectType.MotionBlur]).bloomIntensity);
			}
			if (intensity > 0f)
			{
