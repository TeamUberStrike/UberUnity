...
public class AuthenticationManager : Singleton<AuthenticationManager>
{
	...
	public void LoginByChannel()
	{
		if (ApplicationDataManager.Channel == ChannelType.Steam)
		{
			...
			Debug.Log(string.Format("SteamWorks SteamID:{0}, PlayerPrefs SteamID:{1}", steamId, text));
			if (string.IsNullOrEmpty(steamId))
			{
				Debug.LogWarning("Steam not initialized — skipping login, UberUnity menu will handle gameplay.");
				return;
			}
			if (string.IsNullOrEmpty(text) || text != steamId)
			{
				Debug.Log(string.Format("No SteamID saved. Using SteamWorks SteamID:{0}", steamId));
		...
		{
			Debug.Log("LoginByChannel: WindowsStandalone — login panel not available yet, skipping.");
			// PanelManager may not exist in UberUnity's MainMenu scene.
			// The UberUnity menu system handles gameplay independently.
			// Login will be wired when HaZard provides server credentials.
		}
	}
	...
	private IEnumerator CompleteAuthentication(MemberAuthenticationResultView authView, bool isRegistrationLogin = false)
	{
		...
		if (!PlayerDataManager.IsTestBuild)
		{
			try
			{
				if (UberDaemon.Instance != null)
				{
					PlayerDataManager.MagicHash = UberDaemon.Instance.GetMagicHash(authView.AuthToken);
				}
				else
				{
					Debug.LogWarning("UberDaemon not available, using empty MagicHash");
					PlayerDataManager.MagicHash = string.Empty;
				}
			}
			catch (System.Exception ex)
			{
				Debug.LogWarning("MagicHash computation failed, using empty hash: " + ex.Message);
				PlayerDataManager.MagicHash = string.Empty;
			}
